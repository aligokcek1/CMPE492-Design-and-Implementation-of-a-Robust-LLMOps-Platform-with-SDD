...
- [ ] T002 [P] Initialize Streamlit frontend structure in `frontend/src/app.py`
- [ ] T003 Set up backend `pytest` and `pytest-asyncio` configurations in `backend/pytest.ini`
- [ ] T004 [P] Set up frontend `pytest` configurations for Streamlit testing in `frontend/pytest.ini`
...
- [ ] T008 Create API client wrapper for Streamlit to communicate with FastAPI in `frontend/src/services/api_client.py`
...
- [ ] T012 [US1] Build Login UI component allowing token input and displaying validation errors using TDD (write component tests first using `streamlit.testing`) in `frontend/src/components/auth.py`
- [ ] T013 [US1] Integrate Login UI with backend `/api/auth/verify` via `api_client` and store session state in `frontend/src/app.py`
...
- [ ] T014 [US2] Write contract test for `/api/upload/start` endpoint in `backend/tests/contract/test_upload_api.py`
- [ ] T015 [US2] Implement `LocalModelSession` Pydantic models for upload payload in `backend/src/models/upload.py`
- [ ] T016 [US2] Implement resumable directory upload logic utilizing `huggingface_hub.upload_folder` in `backend/src/services/huggingface.py`
- [ ] T017 [US2] Implement POST `/api/upload/start` endpoint to handle file streams/paths in `backend/src/api/upload.py`
- [ ] T018 [US2] Create Streamlit upload UI component (handling directory/multiple files) and progress indicators using TDD in `frontend/src/components/upload.py`
- [ ] T019 [US2] Integrate upload component with backend upload API, passing auth token and tracking progress in `frontend/src/app.py`
...
- [ ] T020 [US3] Write contract test for `/api/models` endpoint in `backend/tests/contract/test_models_api.py`
- [ ] T021 [US3] Add `list_models` capability to the Hugging Face service in `backend/src/services/huggingface.py`
- [ ] T022 [US3] Implement GET `/api/models` endpoint to retrieve user's existing repositories in `backend/src/api/models.py`
- [ ] T023 [US3] Build Model Selection UI (dropdown or list) fetching data from backend using TDD in `frontend/src/components/upload.py`
- [ ] T024 [US3] Integrate Model Selector into the main workflow, allowing users to bypass upload in `frontend/src/app.py`
...
- [ ] T025 [US4] Write contract test for `/api/deployment/mock` endpoint in `backend/tests/contract/test_deployment_api.py`
- [ ] T026 [US4] Implement `MockDeployment` request/response models in `backend/src/models/deployment.py`
- [ ] T027 [US4] Create mock GCP deployment service that simulates a delay and returns success in `backend/src/services/mock_gcp.py`
- [ ] T028 [US4] Implement POST `/api/deployment/mock` endpoint in `backend/src/api/deployment.py`
- [ ] T029 [US4] Build Deployment Configuration UI (CPU/GPU toggle buttons) using TDD in `frontend/src/components/deploy.py`
- [ ] T030 [US4] Integrate Deployment Configuration UI with mock backend endpoint and display simulated success state in `frontend/src/app.py`

## Phase 7: Polish & Cross-Cutting
*Goal: Ensure end-to-end reliability, style consistency, and handle edge cases outlined in the spec.*

- [ ] T031 Handle edge case: display clear error if Hugging Face token lacks write permissions during upload in `frontend/src/components/upload.py`
- [ ] T032 Handle edge case: Ensure `huggingface_hub.upload_folder` is configured to handle large files gracefully without exceeding available memory (chunked uploads) in `backend/src/services/huggingface.py`
...
- [ ] T034 Add global error handling and `st.toast` / `st.error` notifications for seamless user experience in `frontend/src/app.py`
- [ ] T035 Perform end-to-end integration test of the full workflow (Login -> Upload -> Deploy) using Pytest/Streamlit Testing in `frontend/tests/integration/test_workflow.py`
